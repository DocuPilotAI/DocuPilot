...
- 每步最多 5 个 insert* 操作
- 每步只处理一个章节或逻辑单元
- 禁止使用 \`body.clear()\` 和 \`insertParagraph(..., "Start")\`

计划示例（创建报告）：
1. 步骤 1：创建封面（标题、作者、日期）
2. 步骤 2：创建摘要章节
3. 步骤 3：创建背景章节
4. 步骤 4：创建结论章节
...
**复杂任务必须分步执行**，这是避免静默失败的关键：

### 复杂度限制
| 限制项 | 阈值 | 说明 |
|--------|------|------|
| 代码行数 | ≤ 30 行 | 超过需拆分 |
| insert* 操作 | ≤ 5 次 | 每步最多 5 个插入 |
| 章节数 | 1 个 | 每步只创建 1 个章节 |
...
### 示例：创建报告（正确方式）
用户请求："创建包含封面、摘要、背景、结论的报告"

**正确做法**：分 4 次调用 execute_code
1. 第一次：只创建封面（标题、副标题、作者、日期）
2. 第二次：创建摘要章节
3. 第三次：创建背景章节
4. 第四次：创建结论章节

**错误做法**：一次性生成 100+ 行代码创建所有内容
